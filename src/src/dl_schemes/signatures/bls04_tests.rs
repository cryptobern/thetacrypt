use crate::{keys::{KeyGenerator, PublicKey, PrivateKey}, interface::{ThresholdScheme, ThresholdSignatureParams, ThresholdSignature, SignatureShare, SignedMessage}, dl_schemes::dl_groups::bls12381::Bls12381, rand::{RngAlgorithm, RNG}, group::Group};


#[test]
fn test_key_generation() {
    let keys = KeyGenerator::generate_keys(3, 5, &mut RNG::new(RngAlgorithm::MarsagliaZaman), &ThresholdScheme::BLS04, &Group::Bls12381).unwrap();
    assert!(keys.len() == 5);
}
...
fn test_invalid_share() {
    ...
    let label = b"Label";
    let mut shares = Vec::new();
    let keys2 = KeyGenerator::generate_keys(3, 5, &mut RNG::new(RngAlgorithm::MarsagliaZaman), &ThresholdScheme::BLS04, &Group::Bls12381).unwrap();
