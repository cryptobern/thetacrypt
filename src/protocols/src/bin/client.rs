...
use std::{fs, io};
use std::{thread, time};
use cosmos_crypto::keygen::{PublicKey, ThresholdScheme};
...
//     let key_chain_0: KeyChain = KeyChain::new(); 
//     let key_entry = key_chain_0.get_key(ThresholdScheme::SG02,None).unwrap();
...
//     let key_chain_0: KeyChain = KeyChain::new(); 
//     let key_entry_1 = &key_chain_1.get_key(ThresholdScheme::SG02,None).unwrap();
...
//     let key_chain_0: KeyChain = KeyChain::new(); 
//     let key_entry_2 = &key_chain_2.get_key(ThresholdScheme::SG02,None).unwrap();
...
//     let key_chain_0: KeyChain = KeyChain::new(); 
//     let key_entry_3 = &key_chain_3.get_key(ThresholdScheme::SG02,None).unwrap();
...
//     let share_1 = get_push_share_request(k, &ciphertext, sk_sg02_bls12381_1.clone(), decrypt_response.instance_id.clone());
...
//     let share_1 = get_push_share_request(k, &ciphertext2, sk_sg02_bls12381_1.clone(), decrypt_response2.instance_id.clone());
//     let put_share_response = client.push_decryption_share(Request::new(share_1)).await?;
//     println!("RESPONSE={:?}", put_share_response);
...
//     // let share_1 = get_push_share_request(k, &ciphertext3, sk_sg02_bls12381_1.clone(), decrypt_response3.instance_id.clone());
//     // let put_share_response = client.push_decryption_share(Request::new(share_1)).await?;
//     // println!("RESPONSE={:?}", put_share_response);
...
//     for i in 0..k {
//         shares.push(ThresholdCipher::partial_decrypt(ctxt,&sk[i as usize], &mut params));
//     }
//     shares.shuffle(&mut thread_rng());
//     shares
// }

// fn get_push_share_request(k: u32, ctxt: &Ciphertext, sk: PrivateKey, instance_id: String) -> PushDecryptionShareRequest {
//     let mut params = ThresholdCipherParams::new();
//     // let mut shares = Vec::new();
...
//     // shares.shuffle(&mut thread_rng());
//     // shares
//     let decryption_share = ThresholdCipher::partial_decrypt(ctxt,&sk, &mut params);
