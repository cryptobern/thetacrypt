...
use crate::{keychain::KeyChain, types::{Key, ProtocolError, InstanceStatus, MessageForwarderCommand}, message_forwarder::MessageForwarder, state_manager::StateManager};
...
pub async fn init(rpc_listen_address: String,
    ...
    let (state_command_sender, state_command_receiver) = tokio::sync::mpsc::channel::<StateUpdateCommand>(32);
    ...
    let (forwarder_command_sender, forwarder_command_receiver) = tokio::sync::mpsc::channel::<MessageForwarderCommand>(32);
    ...
    tokio::spawn( async move {
        let mut mfw = MessageForwarder::new(forwarder_command_receiver, incoming_message_receiver, state_command_sender);
        mfw.run().await;
    });
