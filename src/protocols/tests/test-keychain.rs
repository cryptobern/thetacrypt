...
fn test_insert_and_get_key(){
    ...
    // insert a second key 
    let res = key_chain.insert_key(non_default_sg02_bls12381.clone(), non_default_key_id.clone());
    assert!(matches!(res, Ok(_)));
...
fn test_keychain_serialization(){
    ...
    for i in 0..2 {
        ...
        assert_eq!(keys[&key_id].serialize().unwrap(), sk_sg02_bls12381_unser.sk.serialize().unwrap());
    }
}
