use std::{collections::{HashMap, HashSet}, io, vec, thread, time};
...
async fn test_servers_dockerized() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pk = key_chain.get_key_by_scheme_and_group(ThresholdScheme::Sg02, Group::Bls12381)?.sk.get_public_key();
