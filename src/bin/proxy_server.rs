...
use theta_network::{interface::TOBComponentEmpty, network_manager::network_manager::NetworkManager, p2p::gossipsub_setup::static_net::P2PComponent, proxy::proxyp2p::{ProxyConfig, ProxyP2PStub}, types::message::NetMessage};
...
pub async fn start_server(config: &ServerProxyConfig, keychain_path: PathBuf) {
    ...
    let mut network_manager: NetworkManager<NetMessage, ProxyP2PStub, TOBComponentEmpty<NetMessage>> = NetworkManager::new(
        p2n_receiver, 
        n2p_sender, 
        net_config.clone(), 
        my_id, 
