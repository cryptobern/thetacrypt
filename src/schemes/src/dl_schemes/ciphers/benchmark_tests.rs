...
use crate::{keys::KeyGenerator, rand::{RNG, RngAlgorithm}, interface::{ThresholdScheme, ThresholdCipherParams, ThresholdCipher, Ciphertext}, group::Group};
...
fn serialization_old() {
    ...
    for size in msg_sizes {
        ...
        for i in 0..10 {
            let now = Instant::now();
            let bytes = ctxt.serialize_old().unwrap();
            ser_delta += (Instant::now() - now).as_micros();
            
            let now = Instant::now();
            let _ = Ciphertext::deserialize_old(&bytes).unwrap();
            der_delta += (Instant::now() - now).as_micros();
        }
        ser_delta /= 10;
        der_delta /= 10;
